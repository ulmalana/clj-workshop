...
(defn monthly-activity-by-user [db user-id]
  (jdbc/query
   db
   [(str "select au.first_name, au.surname, a.duration, a.activity_type, a.distance, a.activity_date from app_user au, activity a where au.id = a.user_id and a.user_id = " 1)]
   {:row-fn (fn [row]
              (update row :activity_date t/local-date))
    :result-set-fn (fn [rs]
                     (reduce
